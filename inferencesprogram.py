...
openai.api_key = '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'
...
def g():
    ...
    Ginfera = st.text_input("개별적 사실", value = "여러문장 입력시 ,로 구분해서 쓰시오")
    Ginferb = st.text_input("소전제")
    if st.button("입력"):
        content = "귀납법으로 추론, 개별적 사실은 {Ginfer1}, 소전제는 {Ginfer2}, 결론만 출력"

def y():
    st.title("연역법 프로그램 실행합니다.")
    Yinfera = st.text_input("대전제")
    Yinferb = st.text_input("소전제")
    if st.button("입력"):
